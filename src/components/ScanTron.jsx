...
const ScanTron = ({ testKey, testTopic, _id }) => {
  ...
  useEffect(() => {
    const setUp = async () => {
      if (testKey) {
        console.log(testKey);
        // Split the testKey string into an array by commas
        let keyData = testKey.split(",");
        // Update keyArr state with the array of keys
        await setKeyArr(keyData);
        // Update testLength state with the length of the array
        await setTestLength(keyArr.length);
      }
    };
    setUp();
  }, [testKey]);
